'''
Додайте до банкомату меню отримання поточного курсу валют за допомогою requests
...
class ATM:
    ...
    def exchange_rate():
        ...
        if response.ok:
            r = response.json()
            print(f"Курс {r[0]['ccy']}:")
            print(f"Купівля: {round(float(r[0]['buy']), 2)}")
            print(f"Продаж: {round(float(r[0]['sale']), 2)}")
            print("---------------")
            print(f"Курс {r[1]['ccy']}:")
            print(f"Купівля: {round(float(r[1]['buy']), 2)}")
            print(f"Продаж: {round(float(r[1]['sale']), 2)}")
        else:
